...
class ProyeccionInfracTecnolQuerySerializer(serializers.Serializer):
...
class ProyeccionTecnologicaUpdateSerializer(serializers.Serializer):
    llave_id = serializers.CharField(required=True)
    etiquetas_dinamicas = serializers.DictField(child=serializers.DictField())

    def validate(self, data):
        container = self.context["container"]
        obtener_uc = container.proyeccion_tecnologica().obtener_proyeccion_tecnologica()
        proyeccion_tecnologica = obtener_uc.ejecutar(llave_id=data["llave_id"])

        if not proyeccion_tecnologica:
            raise serializers.ValidationError({"error": "Programa no encontrado"})

        data["proyeccion_tecnologica"] = proyeccion_tecnologica

        etiquetas_raw = proyeccion_tecnologica.etiquetas_dinamicas
