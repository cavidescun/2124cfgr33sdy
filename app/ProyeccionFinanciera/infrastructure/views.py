...
from app.ProyeccionFinanciera.infrastructure.input.serializers import EtiquetasDinamicasSerializer, ProyeccionFinancieraQuerySerializer, ProyeccionFinancieraUpdateSerializer
from app.ProyeccionFinanciera.infrastructure.out.serializers import ProyeccionFinancieraDetailResponseSerializer, ProyeccionFinancieraDetailResponseSerializer, ProyeccionFinancieraResponseSerializer, ProyeccionFinancieraUpdateResponseSerializer
...
class ProyeccionFinancieraView(APIView):
    ...
    def patch(self, request):
        serializer =ProyeccionFinancieraUpdateSerializer(data={ "llave_id": request.GET.get("llave_id"), "etiquetas_dinamicas": request.data.get("etiquetas_dinamicas", {})},context={"container": container})
        if  serializer.is_valid():
            ...
            actualizar_uc = container.proyeccion_financiera().actualizar_proyeccion_financiera()
            actualizar_uc.ejecutar(llave_id=proyeccion.llave_maestra,etiquetas_dinamicas=etiquetas_finales,user=request.user)
            response_serializer = ProyeccionFinancieraUpdateResponseSerializer({"llave_id": proyeccion.llave_maestra,"campos_actualizados": campos_actualizados})
            return Response({
            "message": "Proyeccion Proyeccion Financiera actualizada exitosamente",
            "data": response_serializer.data},
                status=status.HTTP_200_OK,)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
